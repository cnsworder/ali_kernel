...
static void update_cfs_shares(struct cfs_rq *cfs_rq)
{
	...
	load_weight = atomic64_read(&tg->load_avg);
...
static inline void __update_task_entity_contrib(struct sched_entity *se)
{
	...
	contrib = se->avg.runnable_avg_sum * (se->load.weight >> 10);
	contrib /= (se->avg.runnable_avg_period + 1);
	se->avg.load_avg_contrib = (contrib << 10);
}
...
{
	__update_entity_runnable_avg(rq->clock, &rq->avg, runnable);
						...
						int wakeup)
{
	...
	if (unlikely(se->avg.decay_count <= 0)) {
		se->avg.last_runnable_update = rq_of(cfs_rq)->clock;
...
static inline u64 cfs_rq_clock_task(struct cfs_rq *cfs_rq)
{
	...
	return rq_of(cfs_rq)->clock - cfs_rq->throttled_clock_task_time;
}
...
static int tg_unthrottle_up(struct task_group *tg, void *data)
{
	...
	if (!cfs_rq->throttle_count) {
		...
		cfs_rq->throttled_clock_task_time += rq->clock -
...
static int tg_throttle_down(struct task_group *tg, void *data)
{
	...
	if (!cfs_rq->throttle_count)
		cfs_rq->throttled_clock_task = rq->clock;
	cfs_rq->throttle_count++;
